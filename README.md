...
- [x] 좋아요 버튼을 누른 데이터만 필터링해서 보여주는 메뉴를 제작한다.
  - [x] 👍 좋아요 버튼을 누른 경우, localStroge의 likedVideoIds 배열에 저장한다.
  - [x] 좋아요 탭으로 이동 시 실행할 이벤트를 정의한다.
  - [ ] 👍 좋아요 버튼을 누른 경우, 좋아요 메뉴 탭에서 동영상을 보여준다.
  - [x] 👍 좋아요 버튼을 다시 클릭한 경우, localStroge의 likedVideoIds 배열에서 해당 동영상의 id를 삭제한다.
  - [ ] 👍 좋아요 버튼을 다시 클릭한 경우, 좋아요 메뉴 탭에서 해당 동영상을 삭제한다.  
- [ ] 스크롤 페이징 방식을 이용해서 Lazy loading을 개선한다.
- [ ] 반응형 웹: 유저가 사용하는 디바이스의 가로 길이에 따라 검색결과의 row 당 column 개수를 변경한다.

### 👾 TODO(TEST) - step3
- [x] 좋아요 버튼을 누르면 해당 탭이 선택되어 하이라이트된다.
  - [x] 👍 좋아요 버튼을 누른 경우, 좋아요 탭에서 해당 동영상을 보여준다.
  - [x] 👍 좋아요 버튼을 다시 클릭한 경우, 좋아요 탭에서 해당 동영상을 삭제한다.
- [ ] 반응형 웹: 유저가 사용하는 디바이스의 가로 길이에 따라 검색결과의 row 당 column 개수가 바뀐다.
