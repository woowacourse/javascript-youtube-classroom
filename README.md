...
- [x] 저장된 영상의 개수가 0인 경우 사용자에게 알린다
  - [x] 볼 영상의 개수가 0인 경우
  - [x] 본 영상의 개수가 0인 경
- [x] modal에서 영상을 저장하면, 볼 영상에 보여준다.
  - [x] 본 영상과 볼 영상을 나누어서 보여준다.
  - [ ] 기본 화면에 보여지는 좋아요, 체크버튼, 휴지통은 선택할 수 있다.
  - [ ] 좋아요는 3단계라는 것을 꼭 PR에 명시한다.
  - [x] 체크버튼을 누르면, 볼영상 < - > 본영상
  - [x] 휴지통 버튼을 누르면 저장된 영상에서 삭제한다.
- [ ] 모든 사용자에게 보여지는 alert && 행동에 대한 결과 → snackbar로 보여준다.
