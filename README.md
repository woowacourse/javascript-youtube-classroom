...
1. 유투브 검색을 선택한다.
2. 유투브 검색창에 보고싶은 영상을 입력한다.
3. 유투브 api를 통해서 결과를 가져온다 => 영상을 가져와야 함(영상, 제작자, 제목, 날짜)

   - 3.1. 결과가 없을 경우 없다는 이미지를 보여준다.
   - 3.2. 결과가 있을 경우, 10개까지만 보여준다. + 스크롤을 내릴때 추가로 가져온다.

4. 검색 결과에서 맘에 드는 영상을 저장할 수 있다.

   - 4.1. 저장을 하면 나만의 유튜브 강의실에 저장한다.
   - 4.2. id를 web storage에 저장한다.
   - 4.3. 저장 버튼을 숨긴다.

5. 다시 검색하기를 누르면, 마지막에 검색한 키워드로 검색한 결과를 보여준다.
   프로그램을 재시작하면 reset해준다.
6. 최근 검색어를 최대 3개까지 보여준다.

   - 6.1. 방금 검색한 단어도 바로 보여준다.
   - 6.2. 최근 검색어를 선택하여 검색결과를 확인할 수 있다.

- UX
  - enter로도 검색이 가능하다.
  - Modal 밖을 선택하면 검색창이 닫힌다.
  - a태그의 글자의 경우 hover로 link임을 표시한다.
  - section tag에 제목을 달아준다.
  - 최근검색어의 a태그들은 list로 묶어준다.
  - 검색어를 한번에 지울 수 있다.
  - 최근검색어를 지울 수 있는 버튼이 있다.
