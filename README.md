...
- [x] (Assertion) 검색어에 맞게 데이터를 잘 가져오는지 확인한다.
- [ ] 사용자가 검색어를 입력하고 검색 버튼을 눌렀을 때, 검색 결과가 화면에 나타난다.
- [ ] 사용자가 검색어를 입력하고 검색 버튼을 눌렀을 때, 최근 검색어에 추가 된다.
  - 검색결과는 가장 왼쪽에 추가되고, 저장된 검색어가 3개 이상인 경우 가장 오른쪽 검색어를 삭제한다.
- [ ] 사용자가 검색어를 아무것도 입력하지 않을 때, 아무반응도 일어나지 않는다.
- [ ] 사용자가 영상 저장 버튼을 누르면, 해당 저장 버튼이 사라지고, 우측 위 저장된 영상 개수가 1 증가한다.
- [ ] 사용자가 영상 저장 버튼을 눌렀는데, 현재 저장된 영상이 100개 인 경우 '저장할 수 없습니다'라는 alert 를 띄운다.
- [ ] 사용자가 스크롤을 내리면, 밑에 영상이 추가된다.
- [ ] 사용자가 모달창을 끄고, 다시 켰을 때 기존 검색 결과가 유지된다.
- [ ] 각 영상이 제목, 작성자, 날짜가 제대로 화면에 표시되는지 확인한다.
