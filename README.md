...
- [ ] 최근 검색어를 클릭하면 그 검색어로 바로 검색할 수 있다.

#### UX 개선 기능 목록

- [ ] 버튼에 focus 혹은 hover 되었을 때의 스타일이 보여지도록 한다.
- [ ] 유튜브 검색 modal을 열었을 때, 검색창에 autofocus되도록 한다.
- [ ] 유튜브 검색 modal이 열려 있을 때, dimmer 영역에 클릭하면 modal이 닫히도록 한다.

### 🎯 step1 테스트 항목

- [ ] `동영상 검색` 버튼을 눌렀을 때, 유튜브 검색 modal이 나타난다.
- [ ] 유튜브 검색 modal에서 검색어를 입력하고 검색 버튼을 누르면, 검색 결과를 10개까지 보여준다.
  - [ ] 예외: 검색어가 비어있는 상태에서 검색 버튼을 누르면, 스낵바 경고 메시지가 나타난다.
- [ ] 유튜브 검색 modal에서 검색 결과 스크롤 바를 끝까지 이동시켰을 경우, 그 다음 10개의 검색 결과를 추가로 보여준다.
- [ ] 검색한 검색어는 최근 검색어에 추가된다.
- [ ] 각 검색 결과 동영상의 `저장` 버튼을 누르면 볼 영상 목록에 동영상이 저장된다.
  - [ ] 예외: 저장된 동영상이 100개 이상일 경우, 스낵바 경고 메시지가 나타난다.
