...
- [ ] 검색 모달창을 열고, 검색창에 검색입력한 후에 10개의 동영상 저장하고 모달을 닫는다.
- [ ] 다시 돌아온 화면에서 "좋아요"를 누른 후, "좋아요 한 비디오 버튼"을 클릭하면, 해당 영상이 보인다.
- [ ] 좋아요 버튼을 다시 클릭하면, 좋아요 버튼이 해지된다.
- [ ] 유저가 가로 길이를 줄이면 그에 맞게 페이지에 보이는 video의 개수가 줄어든다.
