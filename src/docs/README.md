...
- [ ] 내가 볼 영상에 비디오가 있다면, 해당 비디오들이 어떤 것이 있는지 보여준다.
- [ ] 볼 영상 탭이나 본 영상 탭을 클릭하면 각각 해당 비디오들이 어떤 것이 있는지 보여준다.
- [ ] 영상 카드의 이모지 버튼을 클릭하여 아래와 같은 상태 변경이 가능해야 한다.

  - [ ] ✅을 누르면 해당 영상은 본 영상으로 체크한다.
  - [ ] 🗑️을 누르면 저장된 리스트에서 삭제할 수 있다. (삭제 시 사용자에게 정말 삭제할 것인지 물어봅니다.)
