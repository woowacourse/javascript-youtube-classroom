...
- [x] E2E 테스트 코드 작성하기
- [ ] 기능 오류수정
  - [ ] 요구사항가 일치하도록 웹 반응형 수정
  - [ ] "볼 영상"에서 v 체크시 "본 영상"에서 제일 앞이나 뒤에 오지 않음 (기준 설정 필요)
  - [ ] 재검색시 한 번 저장한 목록을 삭제한 경우도 검색목록에 나오지 않음
- [ ] 개선점 (선택)
  - [ ] 볼 영상/본 영상에서 삭제시에도 여전히 검색화면에는 저장된 상태로 표시 (저장버튼 다시노출되지 않음)
  - [ ] 빠르게 스크롤시 Skeleton UI가 실제 조회될 목록보다 많이 그려짐
  - [ ] 볼 영상/본 영상 목록리스트가 너무 짧아서 화면 하단에 불필요한 공백영역 발생
- [ ] 테스트 코드 수정
