...
- [x] 페이지를 방문했을 때 기본 메인 화면으로 내가 볼 영상들의 리스트를 보여준다.
- [x] 본 영상, 볼 영상 버튼을 눌러 필터링 할 수 있다.
- [x] 반응형 웹: 유저가 사용하는 디바이스의 가로 길이에 따라 검색결과의 row 당 column 개수를 변경한다.
  - [x] 1280px 이상: 4개
  - [x] 960px이상~1280px 미만: 3개
  - [x] 600px이상~960px 미만: 2개
  - [x] 600px 미만: 1개
- [x] (추가한 기능) 검색어 입력 시 x 버튼을 누르면 입력한 검색어가 초기화된다.
- [x] (추가한 기능) 모두 삭제 버튼 클릭 시 해당 탭(볼 영상, 본 영상)에서 저장된 모든 영상이 삭제된다. (클릭 시 사용자에게 정말 모두 삭제할 것인지 물어본다.)
- [x] (추가한 기능) 모두 체크 버튼 클릭 시 해당 탭(볼 영상, 본 영상)에서 저장된 모든 영상의 체크 여부가 바뀐다. (클릭 시 사용자에게 모두 체크할 것인지 물어본다.)
...
- [x] 영상 카드의 이모지 버튼을 클릭하여 아래와 같은 상태 변경이 가능해야 한다.
  - [x] ✅ 본 영상으로 체크
  - [x] 🗑️ 버튼으로 저장된 리스트에서 삭제할 수 있다. (삭제 시 사용자에게 정말 삭제할 것인지 물어본다.)
- [x] (추가한 기능) 최근 검색어를 저장한다.
  - [x] (추가한 기능) 해당 검색어 버튼 클릭 시 검색할 수 있다.
