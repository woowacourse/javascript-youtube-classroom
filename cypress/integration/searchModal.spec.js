...
describe("[API를 fixture로 대체] [영상 검색 모달 화면]에서 '영상 검색, 렌더링, 저장' 테스트", () => {
  ...
  it("1. 검색어 입력 후 '검색' 버튼을 누르면, 검색된 영상이 표시된다.", () => {
    cy.searchKeyword(inputKeyword);

    cy.get('.video-item').should('be.visible');
  });

  it('2. 검색 결과 스크롤 바를 끝까지 내리면 추가 영상 10개를 표시한다.', () => {
    cy.searchKeyword(inputKeyword);
  ...
  it("3. '저장' 버튼을 누르면, 선택한 영상의 '저장' 버튼이 '저장됨'으로 변경되어 표시되어야 한다.", () => {
  ...
  it("4. '저장' 버튼을 누르면, '볼 영상' 탭에 해당 영상이 표시되어야 한다.", () => {
    cy.searchKeyword(inputKeyword, 100);
  ...
  it("5. 검색어를 입력하지 않고 '검색' 버튼을 누르면, 영상들이 표시되지 않고 에러 Toast 팝업이 표시되어야 한다.", () => {
  ...
  it('6. 저장된 영상의 개수가 100개를 초과하면 더 이상 저장할 수 없다는 에러 Toast 팝업이 표시되어야 한다.', () => {
    cy.searchKeyword(inputKeyword);
...
describe("[API 사용] [영상 검색 모달 화면]에서 '영상 검색, 렌더링, 저장' 테스트", () => {
  it("7. 검색결과가 없을 검색어를 입력하고 '검색' 버튼을 누르면, 영상들이 표시되지 않고 에러 Toast 팝업이 표시되어야 한다.", () => {
    cy.openSearchModal();
