...
describe('e2e test', () => {
  ...
  it('영상을 저장하면 저장한 영상이 보여야 한다.', () => {
    ...
    cy.get('#saved-video-list').find('.video-card').should('have.length', 1);
  });

  it('본 영상 버튼을 클릭하면 본 영상 필터를 클릭했을 때 영상이 보여야 한다.', () => {
    cy.get('#saved-video-list')
      .find('.video-item__watched-button')
      .first()
      .click();
    cy.get('#saved-video-list').find('.video-card').should('have.length', 0);
