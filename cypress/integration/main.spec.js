...
describe("2. [메인 화면]에서 '저장 목록 관리' 테스트 - 기본", () => {
  beforeEach(() => {
    cy.openSearchModal();
...
describe("2. [메인 화면]에서 '저장 목록 관리' 테스트 - 삭제 관련", () => {
  beforeEach(() => {
    cy.openSearchModal();
    cy.intercept('GET', `${REDIRECT_SERVER_HOST}/${YOUTUBE_SEARCH_PATH}*`, { fixture: 'videoItems' });
  ...
  it("2-2. '볼 영상' 탭에서 '삭제'를 의미하는 이모지(🗑) 버튼을 클릭할 경우, 삭제 의사 재확인 모달이 표시되어야 한다.", () => {
    cy.get('.video-item__button--delete').first().click();
    cy.get('.confirm-modal__content').should('be.visible');
  });

  it("2-3. '삭제 의사 재확인 모달'에서 사용자가 '삭제하기' 버튼을 클릭한 경우, 해당 id의 영상이 '볼 영상' 탭에서 제거되고, 삭제 성공 toast 알림에 표시되어야 한다.", () => {
  ...
  it("2-4. '삭제 의사 재확인 모달'에서 사용자가 '취소하기' 버튼을 클릭한 경우, 해당 영상이 '볼 영상' 탭에서 유지되어야 한다.", () => {
    cy.get('.video-item__button--delete')
      .first()
      .invoke('attr', 'id')
      .then((deletedVideoId) => {
...
describe("2. [메인 화면]에서 '저장 목록 관리' 테스트 - 봤음 체크", () => {
  beforeEach(() => {
    cy.openSearchModal();
    cy.intercept('GET', `${REDIRECT_SERVER_HOST}/${YOUTUBE_SEARCH_PATH}*`, { fixture: 'videoItems' });
    const inputKeyword = '우아한테크코스';
  ...
  it("2-5. '볼 영상' 탭에서 '봤음'을 의미하는 이모지(✅) 버튼을 클릭한 경우, 해당 영상이 '볼 영상' 탭에서 제거되어야 하고, '본 영상'탭에 표시되어야 한다.", () => {
    cy.get('.unwatched-tab')
      .find('.video-item')
      .first()
      .invoke('attr', 'id')
  ...
  it("2-6. '본 영상' 탭에서 음영처리된 '봤음'을 의미하는 이모지(✅) 버튼을 클릭한 경우, 해당 영상이 '본 영상' 탭에서 제거되고, '볼 영상'탭에 표시되어야 한다.", () => {
