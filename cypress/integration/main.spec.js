...
describe('나만의 유튜브 강의실 메인 페이지 테스트', () => {
  ...
  it('페이지 접속시 기본 메인화면이 "볼 영상"인지 테스트한다. ', () => {
    cy.searchAndSave('우테코');
    cy.get('.bg-cyan-100').contains('👁️ 볼 영상');
  });
  ...
  it('영상 카드의 ✅ 버튼 클릭시, 볼 영상의 상태가 본 영상으로 변경되는지 테스트한다.', () => {
    ...
    cy.get('[data-js="saved-page__clip"]').should('be.visible');
  });
  ...
  it('영상 카드의 🗑️ 버튼 클릭시, 저장된 리스트에서 영상이 삭제되는지 테스트한다.', () => {
    ...
    cy.get('[data-js="saved-page__clip"]').should('not.be.visible');
  });
});
